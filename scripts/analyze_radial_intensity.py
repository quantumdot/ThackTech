...
def preprocess_worker(f, args):
    ...
    for i in range(1, len(data.columns)):
        data_stats, bin_edges, binnumber = stats.binned_statistic(data[data.columns[0]], data[data.columns[i]], statistic=args.stat, bins=args.bins, range=[(args.min, args.max)])
        binned_df[data.columns[i]] = data_stats
...
def plot_raw_data(dfs, rbins=500, ibins=500):
    ...
    for d in range(len(dfs)):
        ...
        for i in range(1, df_cols):
            ...
            ip.plot_radial_intensity(ax, df, df.columns[0], df.columns[i], radial_bins=rbins, intensity_bins=ibins)
